...
const RaiseComplaint = () => {
  ...
  const ELECTRICITY_SUBCATEGORY_MAP = {
    "Deductions": [
      "Over consumption",
      "Can't see deduction",
    ],
  ...
  const [alisteSubcategory, setAlisteSubcategory] = useState('');
  const [showAlisteSubCategoryOptions, setShowAlisteSubCategoryOptions] = useState(false);
...
console.log("✅ BOOKINGS ARRAY:", response.data.bookings);
  ...
  const handleSubmit = async () => {
    ...
    if (
      ...
      roomNumber === 'Error loading room'
    ) {
      return Toast.show({
        type: "error",
        text1: "No room assigned",
      });
    }
    ...
    if (!subCategory) {
      return Toast.show({
        type: "error",
        text1: "Sub Category is required",
      });
    }
    ...
    if (
      ...
      return Toast.show({
        type: "error",
        text1: "Sub Sub Category is required",
      });
    }
    ...
    try {
      ...
      formData.append(
        "date",
        complaintDate.toISOString().slice(0, 10)
      );
      ...
      if (uploadedImage) {
        ...
        const filename =
          uploadedImage.split("/").pop() || "photo.jpg";
      ...
      //new
   const requestData = {
  date: complaintDate.toISOString().slice(0, 10),
...
if (category === "Electricity Recharge") {
  requestData.alisteSubcategory = alisteSubcategory;
}

if (uploadedImage) {
  requestData.ticketImage = {
    uri: uploadedImage,
    name: uploadedImage.split("/").pop(),
  };
}
        ...
        <View style={styles.body}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              ...
              setShowAlisteSubCategoryOptions(false);
            }}
          >
            <Text style={styles.floatingLabel}>
              Category
            ...
            <>
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  setShowAlisteSubCategoryOptions(
                    !showAlisteSubCategoryOptions
                  );
